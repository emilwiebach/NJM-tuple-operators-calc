        ...
        if tupsum[i][1] == 0:
            ...
            while j >= 2 and boo == 1:
                if tupsum[i][0] == tupsum[j][0]:
                    tupsum[j][1] = simplify(tupsum[i][1] + tupsum[j][1])
                    boo = 0
                    tupsum.pop(i)
                else:
                    j = j - 1
            if boo == 1:
                tupsum[i][1] = factor(tupsum[i][1])
        i = i - 1
...
def tupsumtolatex(tupsum):
    """
    Generates a string containing LaTex code that can be written into a .txt file and then copied into LaTex in order to display a sum of tuples.
